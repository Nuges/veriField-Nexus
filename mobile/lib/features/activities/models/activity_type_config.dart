...
final List<ActivityTypeConfig> activityTypes = [
  ...
  ActivityTypeConfig(
    ...
    fields: [
      ...
      FormFieldDef(key: 'batch_id', label: 'Batch ID', type: 'string', required: true),
      FormFieldDef(key: 'production_timestamp', label: 'Production Timestamp', type: 'string', required: true),
      FormFieldDef(key: 'kiln_capacity_limit_kg', label: 'Kiln Capacity Limit (kg)', type: 'float', required: true),
      FormFieldDef(key: 'application_timestamp', label: 'Application Timestamp', type: 'string', required: true),
      FormFieldDef(key: 'applied_quantity_kg', label: 'Applied Quantity (kg)', type: 'float', required: true),
      FormFieldDef(key: 'remaining_quantity_kg', label: 'Remaining Quantity (kg)', type: 'float', required: true),
      FormFieldDef(key: 'field_agent_id', label: 'Field Agent ID', type: 'string', required: true),
  ...
  ActivityTypeConfig(
    id: 'EV_TRIP',
    label: 'EV Trip Log',
    description: 'Log electric vehicle fleet trips for diesel displacement',
    icon: Icons.directions_car_rounded,
    color: Colors.purple,
    methodology: 'EV Fuel Displacement Methodology',
    fields: [
      FormFieldDef(key: 'plate_number', label: 'Vehicle Plate Number', type: 'string', required: true),
      FormFieldDef(key: 'trip_distance_km', label: 'Trip Distance (km)', type: 'float', required: true),
      FormFieldDef(key: 'start_location', label: 'Start Location', type: 'string'),
      FormFieldDef(key: 'end_location', label: 'End Location', type: 'string'),
      FormFieldDef(key: 'charging_energy_kwh', label: 'Charging Energy (kWh)', type: 'float'),
    ],
    photos: [
      PhotoFieldDef(
        key: 'trip_log_gps_evidence',
        label: 'GPS Route / Odometer Photo',
        required: true,
        prompt: 'Take a photo of the vehicle dashboard odometer or GPS tracker route.',
      ),
      PhotoFieldDef(
        key: 'charging_receipt',
        label: 'Charging Station Receipt',
        required: false,
        prompt: 'Capture the charging terminal transaction receipt or invoice.',
      ),
    ],
  ),
];
