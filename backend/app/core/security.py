...
async def get_current_user(
    ...
    # Determine if 'sub' is a UUID or an email
    ...
    try:
        ...
        if user is None:
            ...
            user_email = payload.get(
                ...
                "email", user_id if "@" in user_id else f"{user_id}@verifield.local"
            ...
            if is_uuid_sub:
                ...
                # Search by email if UUID didn't match

                email_result = await asyncio.wait_for(

                    db.execute(select(User).where((User.email == clean_email) | (User.email == user_email))),
        ...
        try:
            ...
            logging.getLogger("verifield.security").warning(
                ...
                f"Session rollback failed: {rb_err}"
        ...
        if settings.dev_mode:
            ...
            logging.getLogger("verifield.security").warning(
                ...
                "DEV MODE: DB unreachable, using virtual user from token"
            ...
            try:
                ...
                user_uuid = _uuid.UUID(user_id) if is_uuid_sub else _uuid.uuid4()
            ...
            virtual_user = User(
                ...
                email=payload.get("email", user_id if "@" in user_id else f"{user_id}@verifield.local"),

                full_name=payload.get("email", "Dev User")
...
async def get_optional_user(
    ...
    try:
        ...
        logging.getLogger("verifield.security").warning(
            ...
            f"Database lookup failed in get_optional_user: {e}"
        ...
        try:
            ...
            logging.getLogger("verifield.security").warning(
                ...
                f"Rollback failed in get_optional_user: {rb_err}"
