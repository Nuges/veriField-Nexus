...
async def download_report_pdf(
    ...
    """Securely downloads an authenticated generated PDF report."""
    from fastapi.responses import FileResponse
    filepath, filename, mime_type = await service.get_report_file_for_download(report_id, current_user)
...
async def get_public_overview(db: AsyncSession = Depends(get_db)):
    ...
    """Returns safe public aggregate platform counts for landing page presentation without exposing tenant PII."""

    from sqlalchemy import text

    sectors_count = (await db.execute(text("SELECT COUNT(*) FROM methodology_families"))).scalar() or 0

    meths_count = (await db.execute(text("SELECT COUNT(*) FROM methodologies"))).scalar() or 0

    projects_count = (await db.execute(text("SELECT COUNT(*) FROM projects"))).scalar() or 0

    assets_count = (await db.execute(text("SELECT COUNT(*) FROM assets"))).scalar() or 0

    activities_count = (await db.execute(text("SELECT COUNT(*) FROM activities"))).scalar() or 0

    orgs_count = (await db.execute(text("SELECT COUNT(*) FROM organizations"))).scalar() or 0
...
async def get_report(
    ...
    # Tenant isolation: non-Super Admin can only access their own org's reports
    if current_user.role != "SUPER_ADMIN":
        if not current_user.organization_id or str(report.org_id).lower() != str(current_user.organization_id).lower():
            raise HTTPException(
                status_code=403,
                detail="Forbidden: Cannot access another organization's reports."
            )
...
async def get_metrics_overview(
    ...
    # Tenant isolation: Non-Super Admin cannot query arbitrary organizations
    effective_org_id = current_user.organization_id if current_user.role != "SUPER_ADMIN" else org_id
    metrics = await analytics_service.get_dashboard_metrics(org_id=effective_org_id, sector_id=sector_id)
    return metrics
...
async def get_carbon_ledger(
    ...
    stmt = select(CarbonCalculation, Project).outerjoin(Project, CarbonCalculation.project_id == Project.id)
    ...
    if sector_id:
        ...
        stmt = stmt.where(Project.sector_id == sector_id)



    stmt = stmt.order_by(CarbonCalculation.created_at.desc())
    ...
    for calc, proj in res.all():
        ...
        methodology_code = "DEFAULT"

        if proj and proj.methodology_id:

            from sqlalchemy import text

            meth_res = await db.execute(text("SELECT code FROM methodologies WHERE id = :id"), {"id": proj.methodology_id})

            meth_row = meth_res.fetchone()

            if meth_row:

                methodology_code = meth_row.code
...
async def get_anomalies(
    ...
    if sector_id:
        ...
        stmt = stmt.outerjoin(Asset, Activity.asset_id == Asset.id).outerjoin(Project, Asset.project_id == Project.id)
...
async def resolve_anomaly(
    ...
    if current_user.role not in ("SUPER_ADMIN", "ORG_ADMIN", "PROJECT_MANAGER", "QA_OFFICER"):
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions. Only organization administrators and QA officers can resolve anomalies."
        )
    ...
    if current_user.role != "SUPER_ADMIN":
        if not current_user.organization_id or str(act.organization_id).lower() != str(current_user.organization_id).lower():
            raise HTTPException(
                status_code=403,
                detail="Forbidden: Cannot resolve anomalies belonging to another organization."
            )
...
async def get_trends(
    ...
    stmt = select(Activity.activity_type, func.count(Activity.id).label("count"))
    if current_user.role not in ("SUPER_ADMIN",):
        stmt = stmt.where(Activity.organization_id == current_user.organization_id)
...
async def get_agent_metrics(
    ...
    # Tenant isolation: Restrict to current user's organization for non-Super Admins
    if current_user.role != "SUPER_ADMIN":
        if not current_user.organization_id:
            return {"total_agents": 0, "suspicious_count": 0, "agents": []}
