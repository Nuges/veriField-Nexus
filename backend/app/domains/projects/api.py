...
async def approve_project(
    ...
    if not (has_permission(current_user.role, "compliance:authorize") or has_permission(current_user.role, "org:manage")):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access Denied: Compliance Administrator or Organization Administrator authority required to approve projects."
        )
