...
async def list_sectors(db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": str(f.id), "code": f.code, "name": f.name, "description": f.description, "project_types": f.project_types} for f in families]
...
async def get_sector(sector_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    return {"id": str(family.id), "code": family.code, "name": family.name, "description": family.description, "project_types": family.project_types}
