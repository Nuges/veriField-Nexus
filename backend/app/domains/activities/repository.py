...
class ActivityRepository:
    ...
    async def list_activities_paginated(
        ...
        query = (
            ...
            .outerjoin(UserModel, Activity.user_id == UserModel.id)

            .options(selectinload(Activity.user))

        )

        count_query = select(func.count(Activity.id)).outerjoin(
        ...
        if methodology_id or sector_id:
            ...
            # Since Activity doesn't have project_id directly, join through Asset

            query = query.outerjoin(Asset, Activity.asset_id == Asset.id).outerjoin(Project, Asset.project_id == Project.id)

            count_query = count_query.outerjoin(Asset, Activity.asset_id == Asset.id).outerjoin(Project, Asset.project_id == Project.id)
            ...
            if sector_id:
                ...
                conditions.append(Project.sector_id == sector_id)
