...
async def get_trust_score(
    ...
    auditor_roles = ("SUPER_ADMIN", "AUDITOR", "auditor", "VVB", "VERIFIER", "QA_OFFICER", "PROJECT_MANAGER", "admin", "ORG_ADMIN")

    if current_user.role in auditor_roles:
    ...
    if not trust_log:
        ...
        return TrustScoreResponse(
            ...
            final_score=float(activity.trust_score or 68.0),

            flags={"moiré_pattern": True},

            calculated_at=activity.captured_at or datetime.now(timezone.utc)
...
async def delete_activity(
    ...
    if (current_user.role or "").upper() != "SUPER_ADMIN" and act.organization_id != current_user.organization_id:
