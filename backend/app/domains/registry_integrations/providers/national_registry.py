...
class NationalRegistryProvider(RegistryProvider):
    def __init__(self):
        if not getattr(settings, "ENABLE_VERIFIED_REGISTRY_SYNC".lower(), False):
            raise NotImplementedError("NationalRegistryProvider is disabled by feature flag ENABLE_VERIFIED_REGISTRY_SYNC.")
