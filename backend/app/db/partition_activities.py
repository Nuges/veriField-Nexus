...
async def create_future_partitions(
    ...
    while current_date <= end_limit:
        ...
        start_str = m_start.strftime("%Y-%m-%d 00:00:00+00")

        end_str = m_end.strftime("%Y-%m-%d 00:00:00+00")
    ...
    # Also create DEFAULT partition to guarantee no row ever violates partition key limits

    await session.execute(text("CREATE TABLE IF NOT EXISTS activities_default PARTITION OF activities DEFAULT"))

    logger.info("Ensured partition activities_default is active.")
