...
async def main():
    async with async_session_factory() as db:
        # Find all activities with NULL trust_score
        stmt = select(Activity).where(Activity.trust_score == None)
        result = await db.execute(stmt)
        activities = result.scalars().all()
        
        print(f"Found {len(activities)} activities with NULL trust_score.")
