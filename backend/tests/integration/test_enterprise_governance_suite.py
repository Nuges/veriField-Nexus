...
async def run_enterprise_governance_suite():
    ...
    async for db in get_db():
        ...
        super_admin = sa_res.scalar_one()
        assert super_admin.role == "SUPER_ADMIN", "Super Admin role mismatch"
        print(f"✓ Test A1: Super Admin Authenticated -> {super_admin.email} [PLATFORM Scope]")
        ...
        proj_a = Project(id=uuid.uuid4(), name="Kano Clean Stoves Project", organization_id=org_a.id)
        proj_b = Project(id=uuid.uuid4(), name="Nairobi Biochar Project", organization_id=org_b.id)
        ...
        ), actor_id=str(super_admin.id))
        print(f"✓ Test B2: Provisioned Org Admin -> {admin_a.email} (Org: {org_a.name})")
        ...
        m_res = await db.execute(select(ProjectMembership).where(ProjectMembership.user_id == user_john.id))
        ...
        user_john_db = await auth_repo.get_by_id(user_john.id)
        user_john_db.password_hash = get_password_hash(new_pw)
        user_john_db.requires_password_change = True
        await db.flush()

        # Login with new password
        auth_john = await auth_svc.authenticate(UserLogin(email=user_john.email, password=new_pw))
        assert auth_john is not None, "Login with reset password failed"
        print(f"✓ Test D1: Password Reset Verified -> Authentication succeeded with new password for {user_john.email}")
        ...
        try:
            await auth_svc.authenticate(UserLogin(email=user_john.email, password=new_pw))
        ...
        auth_john_restored = await auth_svc.authenticate(UserLogin(email=user_john.email, password=new_pw))
        ...
        logs_res = await db.execute(select(SecurityAuditLog).where(SecurityAuditLog.target_user_id == user_john.id))
        ...
        # ---------------------------------------------------------------
        await db.execute(text("DELETE FROM security_audit_logs WHERE target_user_id IN (:u1, :u2) OR actor_user_id IN (:u1, :u2)"), {"u1": str(admin_a.id), "u2": str(user_john.id)})
        await db.execute(text("DELETE FROM project_memberships WHERE user_id IN (:u1, :u2)"), {"u1": str(admin_a.id), "u2": str(user_john.id)})
        await db.execute(text("DELETE FROM users WHERE email IN (:e1, :e2)"), {"e1": admin_a.email, "e2": user_john.email})
