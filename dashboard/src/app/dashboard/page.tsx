...
export default function RedesignedDashboardPage() {
  ...
  useEffect(() => {
    ...
    // Set up silent polling every 60 seconds for real-time updates
    ...
    try {
      ...
      if (propertyData && propertyData.properties) {
        ...
        props = props.map((p: any) => {
          let envType = propEnvMap[p.id];
          if (!envType) {
            const isUrbanGeo = p.latitude && p.longitude && p.latitude > 6.3 && p.latitude < 6.7 && p.longitude > 3.0 && p.longitude < 4.0;
            const isUrbanAddr = p.address && (p.address.toLowerCase().includes('lekki') || p.address.toLowerCase().includes('lagos') || p.address.toLowerCase().includes('urban'));
            envType = (isUrbanGeo || isUrbanAddr) ? "URBAN" : "RURAL";
          }
  ...
  const getChartData = () => {
    ...
    isolatedActivities.forEach((act) => {
      ...
      if (ad.usage_hours != null) {
        ...
        // Fallback to default 4.5 hours if not available
        group.usage_hours_sum += 4.5;
        group.usage_hours_count += 1;
      }
  ...
  useEffect(() => {
    const validProps = defaultProperties.filter((p: any) => {
    ...
    if (validProps.length > 0) {
      ...
      setMapSelectedPoint({
        ...
        type: p.environment_type || (mapUrbanToggle ? "URBAN" : "RURAL"),
        co2e: realCo2,
  ...
  }, [properties, activities, isolatedLedger, mapUrbanToggle, selectedPipelineStage]);
  ...
  const triggerVerraExport = async () => {
    ...
    try {
      ...
      console.error(err);
      setLastExportMessage(`Export failed: ${err.message || "Server Error"}`);
    } finally {
