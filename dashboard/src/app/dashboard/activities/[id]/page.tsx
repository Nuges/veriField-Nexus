...
export default function ActivityDetailPage() {
  ...
  const isFlagged = activity.status === "flagged" || activity.status === "rejected";

  const isAudit = activity.status === "audit" || activity.status === "pending_audit" || activity.trust_status === "AUDIT";
  ...
  // Dynamically determine imagery based on payload rather than hardcoded sector strings

  if (actData.baseline_generator_image_url || actData.solar_installation_image_url) {
  ...
  } else if (actData.baseline_fuel_source_image_url || actData.stove_installation_image_url) {

    beforeUrl = actData.baseline_fuel_source_image_url || "";

    beforeLabel = "Baseline State (Before)";
    ...
    afterLabel = "Project Intervention (After)";
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    await import("@/lib/api").then(m => m.updateActivityStatus(id, "audit"));

                    setActivity((prev: any) => prev ? { ...prev, status: "audit", trust_status: "AUDIT" } : null);

                    toast.success("Flagged for VVB Audit", "This activity has been successfully assigned to the Independent VVB Audit Queue.");
