...
export default function ActivitiesPage() {
  ...
  const { activeMethodology, activeProject } = useWorkspace();
  ...
  }, [activeMethodology, activeProject]);
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      // In a fully dynamic setup, we might filter by activeProject or methodology_id here if supported by backend

      const filteredActs = activeProject

        ? res.activities.filter((a: any) => a.project_id === activeProject)

        : res.activities;



      setData({ ...res, activities: filteredActs });
      ...
      setError(err?.message || "Failed to load field activities. Please refresh or re-login.");
  ...
  useEffect(() => {
    ...
    // Auto-refresh the activities list every 60 seconds for near real-time updates
  ...
  }, [page, activityType, status, activeMethodology, activeProject]);
  ...
  const exportActivitiesCSV = () => {
    ...
    const headers = ["ID", "Type", "Status", "Trust Score", "Created At"];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--color-border)] pb-4 animate-fade-in-up">
        ...
        <div>
          ...
          <h1 className="text-xl font-bold tracking-tight text-[var(--color-text-primary)] mt-1">
            ...
            Field Activities Ledger

          </h1>

          <p className="text-[var(--color-text-secondary)] text-xs mt-0.5">

            Review, filter, and audit field activities and metrics.
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#00B47A]/10 text-[#00B47A] text-xs font-bold border border-[#00B47A]/20 hover:bg-[#00B47A]/20 transition-all shadow-sm active:scale-95"
