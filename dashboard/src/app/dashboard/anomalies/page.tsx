...
export default function AnomaliesPage() {
  ...
  const getFlagIcon = (type: string = "") => {
    const safeType = type || "";
    if (safeType.includes("gps") || safeType.includes("travel")) return <MapPin size={14} className="text-blue-400" />;
    if (safeType.includes("time") || safeType.includes("pattern")) return <Clock size={14} className="text-amber-400" />;
    if (safeType.includes("image")) return <AlertCircle size={14} className="text-red-400" />;
                      ...
                      <div className="flex items-center justify-center">
                        {flag.resolved ? (
                          ...
                          <div className="flex items-center justify-center gap-1.5 shrink-0">
                            <button 
                              onClick={() => window.open(`/dashboard/activities/${flag.activity_id}`, "_blank")}
                              className="p-1.5 rounded-lg bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-white border border-blue-500/25 transition-all active:scale-90"
                              title="View Activity Data"
                            >
                              <Search size={13} />
                            </button>
                            <button 
