...
export default function POAPortfolioPage() {
  ...
  const { moduleRegistry, allowedSectors } = useWorkspace();
  ...
  const loadAggregatedData = async () => {
    ...
    try {
      ...
      const ledger = await fetchCarbonLedger().catch(() => null);
      ...
      if (ledger?.data && Array.isArray(ledger.data)) {
        ...
        ledger.data.forEach((c: any) => {
          ...
          if (status === "calculated" || status === "issued" || status === "pending_issuance") {
            ...
            // Find methodology or default to 'unknown'

            let methCode = (c.methodology || c.methodology_used || c.sector || "unknown").toLowerCase();



            yields[methCode] = (yields[methCode] || 0) + val;
  ...
  const triggerVerraExport = async () => {
    ...
    try {
      ...
      setExportMessage(`Export failed: ${err.message || "Server Error"}`);
