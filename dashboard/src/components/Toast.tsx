...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const formatMsg = (msg: any): string | undefined => {
  ...
  const formatTitle = (t: any): string => {
    ...
    if (t && typeof t === "object" && t.message) return String(t.message);
  ...
  const addToast = useCallback((type: ToastType, title: any, message?: any) => {

    const id = `toast-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;

    const cleanTitle = formatTitle(title);

    const cleanMsg = formatMsg(message);

    setToasts((prev) => [...prev, { id, type, title: cleanTitle, message: cleanMsg, duration: type === "error" ? 6000 : 4000 }]);
